...
class WeatherHOC extends PureComponent {
    ...
    fetchWeather = async () => {
        const {onLoading, onLoadComplete, onError, getWeatherReport, selectedUnit, url, provider} = this.props;
        try {
            this.setState({content: onLoading()});
            const weatherInformation = await getWeatherReport(url, provider);
            this.setState({content: onLoadComplete(weatherInformation, selectedUnit)})
        } catch (e) {
    ...
    componentDidUpdate(prevProps) {
        if (this.props.selectedUnit !== prevProps.selectedUnit) {
            const {storedWeatherData, provider, onLoadComplete, selectedUnit} = this.props;
            this.setState({
                ...this.state,
