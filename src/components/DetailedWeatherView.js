...
const DetailedWeatherView = ({
  weatherInformation,
  classes,
  renderTemperatureUnitIn,
}) => {
  const {
    clouds,
    main,
    sys,
    visibility: visibilityMeterValue,
    weather,
    wind,
    dt: timeInUnix,
    name,
  ...
  const windSpeedUnit =
    mainTemperature.unit === TEMPERATURE_UNIT_CELSIUS ? 'm/s' : 'miles/hr';
  const place = `${name}, ${country}`;
  const visiblityForecast = visibilityMeasure(visibilityMeterValue);
  const degreeIcon =
    renderTemperatureUnitIn !== TEMPERATURE_UNIT_KELVIN
      ? renderTemperatureUnitIn.toLowerCase()
      : 'degrees';
      ...
      <div className={classes.card}>
        ...
        <Grid container className={classes.gridRoot} spacing={16}>
              ...
              <div>
                <ListItem>
                  ...
                  <ListItemText
                    inset
                    primary="Sunrise"
                    secondary={getTimeFormatted(sunriseTime)}
                  />
                </ListItem>
                <ListItem>
                  <ListItemIcon style={{ color: 'red' }}>
                  ...
                  <ListItemText
                    inset
                    primary="Sunset"
                    secondary={getTimeFormatted(sunsetTime)}
                  />
                </ListItem>
              </div>
            </Grid>
              ...
              >
                <span>
                  {temperatureConverter(
                    ...
                    mainTemperature.unit,
                    renderTemperatureUnitIn,
                  )}
                </span>
                <i className={`wi wi-${degreeIcon}`} />
              </Typography>
          ...
          <Grid item xs={3}>
            ...
            <ListItem>
              <ListItemIcon style={{ color: 'blue' }}>
                <i className={`wi wi-wind towards-${winDegree}-deg`} />
              </ListItemIcon>
              <ListItemText
                inset
                primary="Wind Speed"
