import {TEMPERATURE_UNIT_KELVIN, TEMPERATURE_UNIT_CELSIUS, TEMPERATURE_UNIT_FAHRENHEIT} from 'config/Constants'
...
export const temperatureConverter = (temperature, sourceType, targetType) => {
    ...
    switch (sourceType.toLowerCase()) {
        case TEMPERATURE_UNIT_KELVIN:
            targetTemperature = targetType.toLowerCase() === TEMPERATURE_UNIT_CELSIUS ? (temperature - 273.15) : (temperature * 1.8) - 459.67;
            break;
        case TEMPERATURE_UNIT_FAHRENHEIT:
            targetTemperature = targetType.toLowerCase() === TEMPERATURE_UNIT_CELSIUS ? (temperature - 32) / 1.8 : (temperature + 459.67) / 1.8;
            break;
        case TEMPERATURE_UNIT_CELSIUS:
            targetTemperature = targetType.toLowerCase() === TEMPERATURE_UNIT_FAHRENHEIT ? (temperature * 1.8) + 32 : (temperature + 273.15);
            break;
        default:
            throw new Error(`Provided source temperature not of type ${TEMPERATURE_UNIT_CELSIUS} or ${TEMPERATURE_UNIT_FAHRENHEIT} or ${TEMPERATURE_UNIT_KELVIN}`)
